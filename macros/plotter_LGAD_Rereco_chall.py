...
cut_xy = "TDCx>0.0 && TDCx<6.0 && TDCy>-6.0 && TDCy<0.0"

cut_ch1 = "amp[0]> 0.04&& amp[0]< 0.4&& risetime[1] > 0.8 && risetime[1] < 1.2&& amp[1]> 0.02&& amp[1]< 0.5&& amp_inverse[1]/amp[1]>-0.7 && TDCx>0.0 && TDCx<6.0 && TDCy>-6.0 && TDCy<0.0"
cut_ch3 = "amp[0]> 0.04&& amp[0]< 0.4&& risetime[3] > 1.15 && risetime[3] < 1.5&& amp[3]> 0.02&& amp[3]< 0.5&& amp_inverse[3]/amp[3]>-0.7 && TDCx>0.0 && TDCx<6.0 && TDCy>-6.0 && TDCy<0.0"
cut_ch4 = "amp[0]> 0.04&& amp[0]< 0.4&& risetime[4] > 1.15 && risetime[4] < 1.5&& amp[4]> 0.02&& amp[4]< 0.5&& amp_inverse[4]/amp[4]>-0.7 && TDCx>0.0 && TDCx<6.0 && TDCy>-6.0 && TDCy<0.0"

cut = cut_ch1 + " && " +cut_ch3 + " && " + cut_ch4


print "\n Final cut -->  "+cut

NEntries_cut=tree.GetEntries(cut)
NEntries_cut1=tree.GetEntries(cut_ch1)
NEntries_cut3=tree.GetEntries(cut_ch3)
NEntries_cut4=tree.GetEntries(cut_ch4)
NEntries_cutxy=tree.GetEntries(cut_xy)

print "\n Num of Events before and after cut: \n"
print str(NEntries)+"   ->   "+str(NEntries_cutxy)+" (xy cut) "
print str(NEntries)+"   ->   "+str(NEntries_cut1)+" (xy cut and ch1 cut) "
print str(NEntries)+"   ->   "+str(NEntries_cut3)+" (xy cut and ch3 cut) "
print str(NEntries)+"   ->   "+str(NEntries_cut4)+" (xy cut and ch4 cut) "
print str(NEntries)+"   ->   "+str(NEntries_cut)+" (xy cut and all channel cut) "


### charge

maxIntegral=0
h_int=ROOT.TH1F("h_int","h_int",100,0,30.0)
h_int.GetXaxis().SetTitle("Integraed Charge [pC]")
h_int.GetYaxis().SetTitle("Events")
h_int.SetTitle("")
tree.Draw("integral[1]+integral[3]+integral[4]>>h_int", cut)
...
myC.Print(options.outputDir+"/"+options.run+"/Ch_combine_int.pdf")
myC.Print(options.outputDir+"/"+options.run+"/Ch_combine_int.png")
myC.Print(options.outputDir+"/"+options.run+"/Ch_combine_int.C")

maxIntegral=0
h_intfull=ROOT.TH1F("h_intfull","h_intfull",100,0,30.0)
h_intfull.GetXaxis().SetTitle("Integraed Charge [pC]")
h_intfull.GetYaxis().SetTitle("Events")
h_intfull.SetTitle("")
tree.Draw("intfull[1]+intfull[3]+intfull[4]>>h_intfull", cut)
...
myC.Print(options.outputDir+"/"+options.run+"/Ch_combine_intfull.pdf")
myC.Print(options.outputDir+"/"+options.run+"/Ch_combine_intfull.png")
myC.Print(options.outputDir+"/"+options.run+"/Ch_combine_intfull.C")

maxAmp=0
h_amp=ROOT.TH1F("h_amp","h_amp",100,0.006,1.0)
h_amp.GetXaxis().SetTitle("Amplitude [V]")
h_amp.GetYaxis().SetTitle("Events")
h_amp.SetTitle("")
tree.Draw("amp[1]+amp[3]+amp[4]>>h_amp", cut)
...
myC.Print(options.outputDir+"/"+options.run+"/Ch_combine_Amp.pdf")
myC.Print(options.outputDir+"/"+options.run+"/Ch_combine_Amp.png")
myC.Print(options.outputDir+"/"+options.run+"/Ch_combine_Amp.C")
...
cut="amp["+options.channelRef+"]> "+str(min_amp_Ref)+"&& amp["+options.channelRef+"]< "+str(max_amp_Ref) + "&& risetime["+options.channel+"] > 0.8 && risetime["+options.channel+"] < 1.2"
...
### rise time
maxRiseTime=0
h_risetime=ROOT.TH1F("h_risetime","h_risetime",100,0.5,1.5)
...
h_risetime.Fit("gaus","","",0.8,1.15)#,low50_risetime-0.2*width_risetime, high50_risetime+0.2*width_risetime)
...
h2_deltaT30_amp=ROOT.TH2F("h2_deltaT30_amp","h2_deltaT30_amp",30,min_amp,max_amp,100,minDeltaT30,maxDeltaT30)
...
h1_deltaT30_amp = ROOT.TH1F("h1_deltaT30_amp","h1_deltaT30_amp",30,min_amp,max_amp)
...
h_deltaT30_WithTimeWalkAndImpactPointCor=ROOT.TH1F("h_deltaT30_WithTimeWalkAndImpactPointCor","h_deltaT30_WithTimeWalkAndImpactPointCor",10000,-50,50)
...
### time walk corrections
h2_deltaT15_amp=ROOT.TH2F("h2_deltaT15_amp","h2_deltaT15_amp",30,min_amp,max_amp,100,minDeltaT15,maxDeltaT15)
...
h1_deltaT15_amp = ROOT.TH1F("h1_deltaT15_amp","h1_deltaT15_amp",30,min_amp,max_amp)
h1_deltaT15_amp = h2_deltaT15_amp.ProfileX()
h1_deltaT15_amp.Draw()
tf1_deltaT15_amp = ROOT.TF1("tf1_deltaT15_amp","[0]+x*[1]",min_amp,max_amp)
...
### time walk corrections
h2_deltaT45_amp=ROOT.TH2F("h2_deltaT45_amp","h2_deltaT45_amp",30,min_amp,max_amp,100,minDeltaT45,maxDeltaT45)
...
h1_deltaT45_amp = ROOT.TH1F("h1_deltaT45_amp","h1_deltaT45_amp",30,min_amp,max_amp)
h1_deltaT45_amp = h2_deltaT45_amp.ProfileX()
h1_deltaT45_amp.Draw()
tf1_deltaT45_amp = ROOT.TF1("tf1_deltaT45_amp","[0]+x*[1]",min_amp,max_amp)
