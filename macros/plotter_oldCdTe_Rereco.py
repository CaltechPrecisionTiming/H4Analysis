...
cut="(!isRinging) && amp["+options.channelRef+"]> "+str(min_amp_Ref)+"&& amp["+options.channelRef+"]< "+str(max_amp_Ref) + "&& risetime[5] > 1.15 && risetime[5] < 1.45"
cutrisetime="(!isRinging) && amp["+options.channelRef+"]> "+str(min_amp_Ref)+"&& amp["+options.channelRef+"]< "+str(max_amp_Ref) 
print "\n cut -->  "+cut
...
maxAmp=0
h_amp=ROOT.TH1F("h_amp","h_amp",100,0.006,0.6)
...
amp_inverseamp_h2=ROOT.TH2F("amp_inverseamp_h2","amp_inverseamp_h2",100,0.006,0.6,100,0.0,0.1)
...
maxAmp=0
h_amp_good=ROOT.TH1F("h_amp_good","h_amp_good",100,0.006,0.6)
h_amp_good.GetXaxis().SetTitle("Amplitude [V]")
h_amp_good.GetYaxis().SetTitle("Events")
h_amp_good.SetTitle("")
tree.Draw("amp["+options.channel+"]>>h_amp_good", "amp_inverse[5]/amp[5]>-0.2")
...
cut = cut + "&& amp["+options.channel+"]> "+str(min_amp)+"&& amp["+options.channel+"]< "+str(max_amp) + "&& amp_inverse["+options.channel+"]/amp["+options.channel+"]>-0.2"
cutrisetime = cutrisetime + "&& amp["+options.channel+"]> "+str(min_amp)+"&& amp["+options.channel+"]< "+str(max_amp) + "&& amp_inverse["+options.channel+"]/amp["+options.channel+"]>    -0.2"
print "\n cut -->  "+cut
...
h_risetime.Fit("gaus","","",1.2,1.5)#,low50_risetime-0.2*width_risetime, high50_risetime+0.2*width_risetime)
...
h_risetime_vs_amp=ROOT.TH2F("h_risetime_vs_amp","h_risetime_vs_amp",100,0,0.5,100,0.5,2.0)
...
h_int.SetTitle("")
tree.Draw("integral["+options.channel+"]/12.0>>h_int", cut)
...
h_intfull.SetTitle("")
tree.Draw("intfull["+options.channel+"]/12.0>>h_intfull", cut)
