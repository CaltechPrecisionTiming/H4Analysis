...
def lxbatchSubmitJob (run, path, cfg, outdir, queue, job_dir, dryrun, file_list, file_path):
    ...
    f.write ('mkdir -p /tmp/H4Analysis_tmp/TOFPET/ \n')
    for file_this in file_list:
    	f.write ('cp '+file_path+file_this.rstrip()+" /tmp/H4Analysis_tmp/"+run+"/"+file_this.rstrip()+' \n')
	file_path_tofpet_this = file_path.replace("H4DAQ","TOFPET")
	f.write (' hadd -k -f /tmp/H4Analysis_tmp/TOFPET/'+run+'.root '+ ' /tmp/H4Analysis_tmp/TOFPET/'+run+'.root ' +file_path_tofpet_this+file_this.rstrip()+' \n')
    f.write ('bin/H4Reco job.cfg '+run+'\n\n')
    f.write ('bin/Rereco ntuples/analysis_'+run+'.root '+'ntuples/analysis_'+run+'_rereco.root '+' -1 --config=cfg/RerecoConfig_BTL_Aug2017.config '+'\n\n')
    f.write ('cd ntuples \n')
    f.write ('cp *'+run+'.root '+outdir+'\n')
    f.write ('cp *'+run+'_rereco.root '+outdir+'\n')
...
if __name__ == '__main__':
    ...
    if len(args.runs) == 1 and os.path.isfile(args.runs[0]):
		...
		os.system("eos ls "+args.inputDir+"/H4DAQ/"+run.rstrip()+" > "+job_dir+"/files_run_"+run.rstrip()+".list")
		...
		if file_list:
			...
			if args.batch == 'lxbatch':
				...
				lxbatchSubmitJob(run.rstrip()+"_"+str(ijob), local_path, args.cfg, stageOutDir, args.queue, job_dir, args.dryrun, job_dir+"/"+run.rstrip()+"_"+str(ijob)+".list", args.inputDir+"/H4DAQ/"+run.rstrip()+"/")
